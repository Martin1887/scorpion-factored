...
CartesianSet AbstractState::regress(
    ...
    /*
      ...
      reasons why a fact V=v can be true in a state after applying an operator o:
      (1) o has no effect <V,v,w>, or
      (2) o has an effect <V,u,v>.

      The first part below takes care of (1) by removing all those values
      from the regression that do occur as effect conditions.

      The second part takes care of (2) by adding for each effect that
      generates v its condition u to the regression.
    */
    for (const FactoredEffectPair &fep : factored_effect_pairs) {
        if (fep.value_before == -1) {
            /* This case captures that we allow all values for a certain
               variable. */
            regression.add_all(fep.var);
            //cout << "added all for " << fep.var << endl;
            continue;
        }
        if (cartesian_set.test(fep.var, fep.value_before)) {
            regression.remove(fep.var, fep.value_before);
        }
    ...
    /*
      Finally, we ensure that if o has a precondition on V, then only the value
      specified in the precondition is viable in the regression through o.
    */

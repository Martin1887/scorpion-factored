...
void verify_factored_effect_task(TaskProxy task) {
    for (OperatorProxy op : task.get_operators()) {
        for (EffectProxy effect : op.get_effects()) {
            // TODO: Are we sure we can handel unconditional effects?
            int num_effect_conditions =
                static_cast<int>(effect.get_conditions().size());
            if (num_effect_conditions > 1) {
                cerr << "This configuration expects factored effect tasks but "
                     << "operator " << op.get_name() << " has effects with "
                     << "more than one condition.\nTerminating." << endl;
                utils::exit_with(ExitCode::SEARCH_UNSUPPORTED);
            } else if (num_effect_conditions == 1) {
                int condition_var =
                    effect.get_conditions()[0].get_variable().get_id();
                int effect_var = effect.get_fact().get_variable().get_id();
                if (condition_var != effect_var) {
                    cerr << "This configuration expects factored effect tasks "
                         << "but operator " << op.get_name() << " has an "
                         << "effect conditioned on a variable other than its "
                         << "effect.\nTerminating." << endl;
                    utils::exit_with(ExitCode::SEARCH_UNSUPPORTED);
                }
            }
